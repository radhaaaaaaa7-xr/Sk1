...
def player_info():
    ...
    try:
        ...
        return jsonify({
            "result": 0,
            "UID": q_params.get('account_id', ['Unknown'])[0],
            "Nickname": q_params.get('nickname', ['Unknown'])[0],
            "Region": q_params.get('region', ['Unknown'])[0]
        })
...
def bind_info():
    ...
    try:
        ...
        # Timing ko Days/Hours me convert karne ka logic
        e = data.get("email", "None")
        etb = data.get("email_to_be", "None")
        cd = data.get("request_exec_countdown", 0)
        
        formatted_data = {
            "result": 0,
            "Current_Email": e if e else "None",
            "Pending_Email": etb if etb else "None"
        }
        ...
        if etb and cd > 0:
            ...
            h, m = divmod(h, 3600)
            m, s = divmod(m, 60)
            formatted_data["Time_Remaining"] = f"{d} Days, {h} Hours, {m} Mins, {s} Secs"
...
def send_otp():
    ...
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:send_otp", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": "Timeout Error. Garena blocked the IP."})
...
def verify_otp():
    ...
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:verify_otp", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def bind_email():
    ...
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:create_bind_request", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def verify_identity():
    ...
    if data.get('otp'): payload['otp'] = data.get('otp')
    elif data.get('sec_code'): payload['secondary_password'] = hashlib.sha256(data.get('sec_code').encode('utf-8')).hexdigest()
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:verify_identity", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def change_email():
    ...
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:create_rebind_request", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def unbind_email():
    ...
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:create_unbind_request", headers=get_headers(), data=payload, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def cancel_bind():
    data = request.json
    try:
        r = requests.post("https://100067.connect.garena.com/game/account_security/bind:cancel_request", headers=get_headers(), data={"app_id": "100067", "access_token": data.get('token')}, timeout=15)
        return jsonify(r.json())
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def bound_platforms():
    data = request.json
    try:
        r = requests.get("https://100067.connect.garena.com/bind/app/platform/info/get", params={"access_token": data.get('token')}, headers=get_headers(), timeout=15)
        d = r.json()
        P_MAP = {1: "Garena", 3: "Facebook", 4: "Guest", 5: "VK", 8: "Google", 11: "X (Twitter)", 13: "Apple ID"}
        return jsonify({"result": 0, "platforms": [P_MAP.get(p_id, f'Unknown ({p_id})') for p_id in d.get("bounded_accounts", [])]})
    except Exception as e: return jsonify({"result": 1, "error": str(e)})
...
def eat_to_token():
    ...
    try:
        res = requests.get(f"https://api-otrss.garena.com/support/callback/?access_token={eat_url}", headers={"User-Agent": "Mozilla/5.0"}, allow_redirects=True, timeout=15)
        return jsonify({"result": 0, "access_token": urllib.parse.parse_qs(urllib.parse.urlparse(res.url).query).get('access_token', [''])[0]})
...
def login_history():
    ...
    try:
        r = requests.post("https://client.ind.freefiremobile.com/GetLoginHistory", headers={"Authorization": f"Bearer {jwt_token}", "Content-Type": "application/x-www-form-urlencoded"}, data=enc(b""), verify=False, timeout=15)
        d = dec(r.content)
        records, offset = [], 0
        while offset < len(d):
